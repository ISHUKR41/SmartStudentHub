...
export function setupGoogleAuth(app: Express) {
  ...
  if (isDevelopment && (googleConfig.clientID === 'dummy-client-id' || googleConfig.clientSecret === 'dummy-client-secret')) {
    console.log('🔧 Development mode: Google OAuth credentials not configured, using mock authentication');
  ...
  passport.use(new GoogleStrategy({
    ...
    try {
      ...
      if (googleUser.email.includes('@faculty.') || googleUser.email.includes('@staff.')) {
        role = 'faculty';
...
export function isAuthenticated(req: any, res: any, next: any) {
  if (isDevelopment && (!process.env.GOOGLE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID === 'dummy-client-id')) {
